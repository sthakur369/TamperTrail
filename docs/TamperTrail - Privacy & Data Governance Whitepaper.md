...
| Principle | TamperTrail Implementation |
|-----------|----------------------|
...
| Scenario | Traditional Log Viewer | TamperTrail |
|----------|----------------------|---------|
...
| Log forensic operations in TamperTrail itself | Use the audit system to audit the auditors |
...
| Obligation | Action in TamperTrail |
|-----------|-----------------|
...
| Document retention decisions | Maintain a data retention register mapping TamperTrail environments to retention schedules |
...
| Article | Requirement | TamperTrail Control | Customer Obligation |
|---------|-------------|-----------------|-------------------|
...
| Art. 30 | Records of processing | — | Include TamperTrail in your ROPA if personal data is present |
| Art. 32 | Technical measures | AES-128, Argon2id, rate limiting, RLS, hash chain | Provision TLS, host security, key management |
| Art. 33/34 | Breach notification | Hash chain detects unauthorized modification; no automated alerts | Establish monitoring process; TamperTrail does not send breach alerts |
...
| Obligation | TamperTrail Relevance | Customer Action |
|-----------|------------------|----------------|
| Right to Know | TamperTrail records may contain personal information (actor fields, IP) | Establish DSAR process covering TamperTrail records; map `actor` conventions to identities |
| Right to Delete | Applies if TamperTrail holds personal information not exempt as business records | Use partition deletion or crypto-shredding for targeted erasure |
| Right to Opt-Out of Sale | TamperTrail holds no data outside customer infrastructure | Zero data egress — not applicable to TamperTrail itself |
...
| Safeguard | TamperTrail Control | Gap / Customer Action |
|-----------|----------------|----------------------|
...
| Control | TamperTrail Implementation |
|---------|----------------------|
...
| **No automated PII detection** | TamperTrail does not scan `tags` for inadvertently placed PII | Data misclassification by engineers is entirely undetected and unblocked |
...
| **No formal compliance certification** | TamperTrail has not undergone a third-party SOC 2 audit | This document provides controls mapping; formal attestation requires independent auditor |
