...
| **Single-worker process** | VeriLog runs as a single Uvicorn worker (`--workers 1`). Concurrency is handled via Python's `asyncio` event loop, and hash chain integrity is enforced at the database level via PostgreSQL advisory locks. This is intentional — a single worker eliminates inter-process state contention and keeps memory usage predictable. | No action required. Do not increase the worker count — it increases memory usage without throughput benefit, as the bottleneck is disk I/O and database commit latency, not Python concurrency. |
